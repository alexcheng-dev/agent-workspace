...
start_tunnel() {
  ...
  [[ "$name" == "worker-agents" ]] && status_file="${LOLGAMES_TUNNEL_STATUS_FILE:-/tmp/lolgames-tunnel-status.json}"
  local status_args=()
  if [[ "$name" == "worker-agents" ]]; then
    local status_http="${LOLGAMES_TUNNEL_STATUS_HTTP:-127.0.0.1:$((port + 1))}"
  ...
  if ! grep -Fq "$public_url" "$log_path" 2>/dev/null; then
    echo "Tunnel registration did not confirm for $public_url" >&2
    return 1
  fi
  for _ in $(seq 1 30); do
  ...
  echo "Tunnel public probe failed for ${public_url}${probe_path}" >&2
  return 1
}
...
sync_worker_app() {
  ...
  if [[ -n "$WORKER_ARCHIVE_PATH" && -f "$WORKER_ARCHIVE_PATH" ]]; then
    ...
    git clone --depth 1 "$WORKER_AGENTS_GIT_URL" "$target_app_home"
  else
...
EOF
  ...
  TMUX='' tmux -L workeragents -f /dev/null new-session -d -s workeragents "cd \"$APP_HOME\" && PORT=${APP_PORT} AGENT_CONSOLE_HOST=127.0.0.1 HERMES_WEBUI_DIR=\"$HERMES_WEBUI_HOME\"${AGENT_AUTO_START_ALL:+ AGENT_AUTO_START_ALL=\"$AGENT_AUTO_START_ALL\"} npm start > ~/worker-agents.log 2>&1"
fi
